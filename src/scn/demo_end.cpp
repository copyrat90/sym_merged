...
{
    _lang_changed_observer.callback.set<&demo_end::on_lang_changed>(*this);
    _lang_changed.attach(_lang_changed_observer);
}

bool demo_end::update(scene_stack&)
{
    return false;
}

void demo_end::on_lang_changed(ldtk::gen::lang_kind lang)
{
    ((void)lang);
}

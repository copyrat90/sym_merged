...
constexpr ZoneInfo::ButtonInfo Z0_hoverButtonInfos_[] = {
    {{247 - MAP_DIMENSIONS[0].width() / 2, 109 - MAP_DIMENSIONS[0].height() / 2}, false, 0},
};
...
constexpr ZoneInfo::ButtonInfo Z1_hoverButtonInfos_[] = {
    {{358 - MAP_DIMENSIONS[0].width() / 2, 56 - MAP_DIMENSIONS[0].height() / 2}, false, 0},
};

constexpr ZoneInfo::ButtonInfo Z1_pressureButtonInfos_[] = {
    {{490 - MAP_DIMENSIONS[0].width() / 2, 64 - MAP_DIMENSIONS[0].height() / 2}, false, 1},
};

constexpr bn::array<bn::string_view, 2> Z1_signTexts_English[] = {
    {"L/R: Pick up a symbol"},
    {"Switches open the doors", "with the same number."},
    {"You can't use the hand", "holding the symbol"},
    {"Down+L/R: Put the symbol down"},
};

constexpr bn::array<bn::string_view, 2> Z1_signTexts_Korean[] = {
    {"L/R:문양 집기"},
    {"스위치의 번호와 같은", "번호의 문이 열린다"},
    {"문양을 든 손으론", "스위치를 누르지 못한다"},
    {"아래+L/R:내려놓기"},
};
...
constexpr ZoneInfo::SymbolInfo Z2_symbolInfos_[] = {
    {{170 - MAP_DIMENSIONS[0].width() / 2, 356 - MAP_DIMENSIONS[0].height() / 2}, entity::Symbol::Type::BAR, false},
    {{193 - MAP_DIMENSIONS[0].width() / 2, 356 - MAP_DIMENSIONS[0].height() / 2}, entity::Symbol::Type::XOR, false},
};
...
constexpr ZoneInfo::ShutterInfo Z2_shutterInfos_[] = {
    {{303 - MAP_DIMENSIONS[0].width() / 2, 264 - MAP_DIMENSIONS[0].height() / 2}, false, 0},
    {{313 - MAP_DIMENSIONS[0].width() / 2, 264 - MAP_DIMENSIONS[0].height() / 2}, false, 1},
};
...
constexpr ZoneInfo::ButtonInfo Z2_pressureButtonInfos_[] = {
    {{217 - MAP_DIMENSIONS[0].width() / 2, 272 - MAP_DIMENSIONS[0].height() / 2}, false, 0},
    {{262 - MAP_DIMENSIONS[0].width() / 2, 272 - MAP_DIMENSIONS[0].height() / 2}, false, 1},
};

constexpr bn::array<bn::string_view, 2> Z2_signTexts_English[] = {
...
constexpr bn::array<bn::string_view, 2> Z2_signTexts_Korean[] = {
...
constexpr ZoneInfo::SignInfo Z2_signInfos_[] = {
    {{83 - MAP_DIMENSIONS[0].width() / 2, 311 - MAP_DIMENSIONS[0].height() / 2},
     Z2_signTexts_English[0],
     Z2_signTexts_Korean[0]},
    {{130 - MAP_DIMENSIONS[0].width() / 2, 352 - MAP_DIMENSIONS[0].height() / 2},
     Z2_signTexts_English[1],
     Z2_signTexts_Korean[1]},
    {{240 - MAP_DIMENSIONS[0].width() / 2, 264 - MAP_DIMENSIONS[0].height() / 2},
     Z2_signTexts_English[2],
     Z2_signTexts_Korean[2]},
    {{52 - MAP_DIMENSIONS[0].width() / 2, 272 - MAP_DIMENSIONS[0].height() / 2},
...
constexpr ZoneInfo::ExitInfo Z2_exitInfos_[] = {
    ...
    {{327 - MAP_DIMENSIONS[0].width() / 2, 264 - MAP_DIMENSIONS[0].height() / 2, 2, 32}, 3, 0},
};

constexpr ZoneInfo::EntranceInfo Z2_entranceInfos_[] = {
    {{27 - MAP_DIMENSIONS[0].width() / 2, 272 - MAP_DIMENSIONS[0].height() / 2}},
    {{276 - MAP_DIMENSIONS[0].width() / 2, 264 - MAP_DIMENSIONS[0].height() / 2}},
};
...
constexpr ZoneInfo zoneInfos_[] = {
    ...
    ZoneInfo(bn::affine_bg_items::bg_w1_s0_0, Z2_symbolInfos_, Z2_doorInfos_, Z2_shutterInfos_, Z2_hoverButtonInfos_,
             ...
             helper::tilemap::IndexRect({0, 23}, bn::size{41, 25}, bg_w1_s0_0.map_item().dimensions()), Z2_signInfos_,
             Z2_exitInfos_, Z2_entranceInfos_),
};

constexpr StageInfo w1_s0_("Tutorial", "Welcome to sym_merged!", false, bn::music_items::music_detour, zoneInfos_);
